"""Meta primitive: active_days.

Count of distinct UTC calendar days on which the actor sent at least one
message. Always <= corpus_span_days. Use alongside activity_density for
presence shape. See BEHAVE-TEXT 0.1.2 `primitives.py` lines 139-146.
"""
...
PRIMITIVE_NAME: str = "meta.active_days"
...
def compute(
    *,
    ...
    return Observation(
        ...
        source=f"eyenet/sensor/primitives/meta-active-days:v{PRIMITIVE_VERSION}#{CONFIDENCE_HEURISTIC_TAG}",
        evidence_ref=None,
        ts=time.time(),
    )
