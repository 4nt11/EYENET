...
"""POST /v1/candidates/{candidate_id}/retry — failed -> queued (M9.D3).

``admin:candidates`` (grant-only): retrying a platform-rejected join can burn
identities, so it's gated above ordinary triage.
"""
...
@router.post(
    ...
    operation_id="candidates_retry",
    response_model=CandidateDetail,
    status_code=202,
)
async def candidates_retry(
    candidate_id: UUID,
    current_user: Annotated[CurrentUser, Depends(RequireScope("admin:candidates"))],
    ...
    await audit.emit(
        event="eyenet.audit.candidate.retry",
