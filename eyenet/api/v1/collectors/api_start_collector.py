...
"""POST /v1/collectors/{collector_id}/start — request RUNNING (M9.D2).

Sugar for ``PATCH {desired_state: RUNNING}``. 202: the desired_state is set
synchronously, but the supervisor reconciles ``observed_state`` off-path — the
returned detail shows the new desired_state with observed_state still pending.
"""
...
@router.post(
    "/collectors/{collector_id}/start",
    operation_id="collectors_start",
    response_model=CollectorDetail,
    status_code=202,
)
async def collectors_start(
    collector_id: UUID,
    ...
    row = await storage.set_collector_desired_state(
        collector_id=collector_id, desired_state=CollectorDesiredState.RUNNING
    )
    await audit.emit(
        event="eyenet.audit.collector.start_requested",
        subject_kind="collector",
        subject_id=collector_id,
        system_user_id=current_user.user_id,
        payload={"desired_state": CollectorDesiredState.RUNNING.value},
    )
